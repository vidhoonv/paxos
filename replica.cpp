...
#define PERFORM_COMMAND(command) \
			...
			{	\
				...
				/*TEST CASE REPLICA FAILURE BEFORE SENDING READ COMMIT		\
						if(my_pid == 0 && client_cmdtype_map[command] == READ_COMMAND)	\
							exit(-1);		\	*/	\
				/*TEST CASE REPLICA FAILURE BEFORE SENDING WRITE COMMIT	\ 	\
						if(my_pid == 0 && client_cmdtype_map[command] != READ_COMMAND)	\
							exit(-1);						*/\
