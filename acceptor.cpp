...
int REPLICA_PORT_LIST[MAX_REPLICAS] = {2000,2002};//,2004};//,2006,2008};
...
//setup commander addresses
	...
	for(i=0;i<MAX_COMMANDERS;i++)
	{
		...
		commander_addr_in[i]->sin_port  = htons(COMMANDER_PORT_STARTER+i);  
...
//setup scout addresses
	for(i=0;i<MAX_SCOUTS;i++)
	{
		...
		scout_addr_in[i]->sin_port  = htons(SCOUT_PORT_STARTER+i);
